...
In the CSV file used as an input to `bluerecording.writeH5_prelim.initialize_h5_file()`, electrodes using the objective CSD methods can be specified by setting the value of the `type` column to `ObjectiveCSD_Sphere`, `ObjectiveCSD_Disk`, or `ObjectiveCSD_Plane`. BlueRecording will use the following defaults:
